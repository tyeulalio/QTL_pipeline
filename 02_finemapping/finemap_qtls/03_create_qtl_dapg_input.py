...
def get_args():
    ...
    parser.add_argument('-p', '--pheno_file', type=str, required=True, help="Phenotype file separated by chromosome, use * for chrom number")
...
def process_genotype(sbamfile, priorsfile, ordered_samples, gene, missing_priors_file, torus_dir, geno_file):
    ...
    # get snp chromosome - all should be on same chromosome
    chrom = snplist[0].split('_')[0].replace("chr", "")
    print(chrom)
    print(snplist[1])
    print(geno_file)

    geno_file = geno_file.replace("*", chrom)
    print(geno_file)

    # work with snps in batches or else grep can throw error
    batch_size = 3000
    num_batches = int(numsnps / batch_size) + 1
    lines = ""
    for i in range(0,num_batches):
        # grep the geno file for these snps
        start_idx = i * batch_size
        end_idx = (i+1) * batch_size
        i_snps = snplist[start_idx:end_idx]
        print("start", start_idx, "end", end_idx)

        snp_str = '|'.join(i_snps)
        cmd = "grep -E '{}' {}".format(snp_str, geno_file)
        process = subprocess.Popen(cmd.split(' '),
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()

        lines = lines + stdout.decode('utf-8')

    lines = lines.rstrip().split('\n')
    print("num lines", len(lines))

    # create a dictionary for these snps
    genotypes_dict = {}
    for line in lines:
        # split on tab
...
def main():
    ...
    process_genotype(sbamfile, priorsfile, ordered_samples, gene, missing_priors_file, args.torus_dir, args.geno_file)
