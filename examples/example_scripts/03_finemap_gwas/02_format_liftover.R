...
savedir <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/02_check_liftover/")
savedir <- paste0("../../example_output/03_gwas/")
dir.create(savedir)


gwas_types <- c('wightman', 'kunkle', 'trubetskoy')
gwas_type <- gwas_types[1]
savedir <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/02_check_liftover/", gwas_type, "/")
dir.create(savedir)
...
join_gwas_qtls <- function(gwas, qtl_snps, gwas_type){
    ...
    #qtl_gwas_snps <- readRDS(savefile)
...
load_gwas_scores <- function(final_matched){
    ...
    if (gwas_type == 'kunkle'){
        ...
        gwas_scores_hg37 <- read_delim(datafile) 
        ...
        filtered_scores_hg37 <- gwas_scores_hg37 %>%
            ...
            unite(gwas_name, Chromosome, Position, Effect_allele, Non_Effect_allele, sep='_') %>%
            inner_join(final_matched)
        head(filtered_scores_hg37)

        # format the dataframe, rename columns
        formatted_scores <- filtered_scores_hg37 %>%
            select(chrom, pos, qtl_var=var, gwas_beta=Beta, gwas_SE=SE, gwas_pval=Pvalue, marker_name=MarkerName, 
                   qtl_ref=ref.qtl, qtl_alt=alt.qtl, gwas_ref=ref.gwas, gwas_alt=alt.gwas, gwas_hg37_var=gwas_name) %>%
            separate(qtl_var, c('qtl_chrom', 'qtl_pos', 'ref', 'alt'), sep='_') %>% # get the ref and alt
            select(-qtl_chrom, -qtl_pos) %>%
            separate(gwas_hg37_var, c('gwas_chrom', 'gwas_pos', 'gwas_effect', 'gwas_non_effect'), sep='_') %>%
            select(-gwas_chrom, -gwas_pos)
        head(formatted_scores)

        # need to swap sign of gwas_beta if effect != alt
        updated_scores <- formatted_scores %>%
            select(-qtl_ref, qtl_alt) %>%
            mutate(updated_beta = ifelse(gwas_effect != alt, -gwas_beta, gwas_beta))
        head(updated_scores)

        savefile <- paste0(savedir, "gwas_updated_scores.csv")
        write_csv(updated_scores, savefile)

        updated_scores <- read_csv(savefile)
        ...
        dgap_scores <- updated_scores %>%
            ...
            mutate(gwas_zscore2 = updated_beta / gwas_SE) %>%
            mutate(gwas_zscore3 = qnorm(1-gwas_pval/2)) %>% # compute zscore
            mutate(gwas_zscore = gwas_zscore * sign(updated_beta),
                   gwas_zscore3 = gwas_zscore3 * sign(updated_beta)
            ) 
        head(dgap_scores, 10)
        dim(dgap_scores)

        dgap_scores %>%
            filter(round(gwas_zscore,2) != round(gwas_zscore3,2)) %>%
            head()
        dim(dgap_scores)
        sum(dgap_scores$gwas_zscore == dgap_scores$gwas_zscore3)
        ...
        final_scores = dgap_scores %>%
            select(var=var_id, zscore=gwas_zscore)
    ...
    if (gwas_type == "trubetskoy"){
        datafile <- paste0("/home/eulalio/deconvolution/data/ad_gwas/trubetskoy_etal_2022_schizo_european.v3.vcf.tsv")
        gwas_scores_hg37 <- read_delim(datafile, comment='#',
                                       col_names=c("chrom", "id", "pos", "ref", "alt",
                                                   "fcas", "fcon", "impinfo",
                                                   "beta", "se", "pval", "ncas",
                                                   "ncon", "neff"
                                       )
        ...
        head(gwas_scores_hg37)
        filtered_scores_hg37 <- gwas_scores_hg37 %>%
            mutate(chrom = paste0("chr", as.character(chrom))) %>%
            unite(gwas_name, chrom, pos, ref, alt, sep='_') %>%
            inner_join(final_matched)
        head(filtered_scores_hg37)
        ...
        # get the zscore
        formatted_scores <- filtered_scores_hg37 %>%
            select(chrom, pos, qtl_var=var, gwas_beta=beta, gwas_SE=se, gwas_pval=pval, marker_name=id, 
                   qtl_ref=ref.qtl, qtl_alt=alt.qtl, gwas_ref=ref.gwas, gwas_alt=alt.gwas, gwas_hg37_var=gwas_name) %>%
            separate(qtl_var, c('qtl_chrom', 'qtl_pos', 'ref', 'alt'), sep='_', remove=FALSE) %>% # get the ref and alt
            select(-qtl_chrom, -qtl_pos) %>%
            separate(gwas_hg37_var, c('gwas_chrom', 'gwas_pos', 'gwas_effect', 'gwas_non_effect'), sep='_') %>%
            select(-gwas_chrom, -gwas_pos)
        head(formatted_scores)
        ...
        # create the zscore
        zscores <- updated_scores %>%
            mutate(zscore = updated_beta/gwas_SE)
        head(zscores)
        ...
        final_scores <- zscores %>%
            select(var=qtl_var, zscore)
        head(final_scores)
        summary(final_scores$zscore)
    }
...
create_ld_annotations <- function(){
    ...
    datafile <- paste0("/home/eulalio/deconvolution/data/ld_blocks/deCODE_EUR_LD_blocks.bed")
...
load_gwas_snps <- function(gwas_type){
    ...
    datafile <- NA
    if (gwas_type == 'kunkle'){
        datafile <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/01_liftover_gwas/Kunkle_etal_2019_Stage1_results_hg38.vcf")
    }
    if (gwas_type == 'wightman'){
    ...
    if (gwas_type == 'trubetskoy'){
        datafile <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/01_liftover_gwas/trubetskoy_2022_schizo_european_results_hg38.vcf")
    }
    datafile
